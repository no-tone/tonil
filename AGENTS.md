...
- `bun run test` enforces per-package coverage thresholds (`vitest.config.ts`'s `coverage.thresholds`), `bun run check-bundle-size` guards each Worker's bundled-script size, and `bun run e2e` (Playwright, `packages/playwright-config`) smoke-tests `apps/web`/`apps/dashboard` against a real `wrangler dev` - all three run in CI; see [docs/deployment.md](./docs/deployment.md) for the bundle-size budget.
