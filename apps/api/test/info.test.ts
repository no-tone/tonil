...
describe("GET /info/:slug", () => {
  it("renders the JSX HTML page for browsers", async () => {
    ...
    expect(await res.text()).toContain("<h1>no-tone</h1>");
  });

  it("serves the markdown variant to agents", async () => {
    const res = await SELF.fetch("https://api.no-tone.com/info/dashboard", {
  ...
  it("404s for an unknown slug", async () => {
    const res = await SELF.fetch("https://api.no-tone.com/info/nonexistent");
