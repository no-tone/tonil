...
describe("apps/web middleware", () => {
  ...
  it("redirects a trailing-slash URL to the canonical form", async () => {
    ...
    // that form.
    const ctx = buildContext("https://no-tone.com/cv/?x=1");
    const res = await run(ctx);
    expect(res.status).toBe(308);
    expect(res.headers.get("Location")).toBe("https://no-tone.com/cv?x=1");
  });
  ...
  it("upgrades a plaintext www request to https rather than mirroring it", async () => {
    ...
    const ctx = buildContext("http://www.no-tone.com/projects?a=1");
    const res = await run(ctx);
    expect(res.status).toBe(301);
    expect(res.headers.get("Location")).toBe(
      "https://no-tone.com/projects?a=1",
    );
  });
